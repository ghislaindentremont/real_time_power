...
##### We check for any time lags or other timing issues. Interstingly there appears to be a consistent lag at the end of blocks, but only for the cue trial stage. Additionally, there are a few scattered timing mishaps for the ME fixation time stage. 
...
raw_long_fil %>%
  dplyr::filter(block == 2, trial < 10) %>%
  ggplot(aes(y = and_notch_filtered, x = time_adj))+
    geom_line()+
    facet_grid(channel ~block_cond)+
...
pwr_cal = raw_long_pwr %>%
  dplyr::filter(block != "1", !(block == 3 & trial == 42 & block_cond == "MI")) %>%
  group_by(block_cond, block, trial, trial_stage, channel) %>%
...
pwr_compare %>% 
  ggplot(aes(x = avg_cal, y = avg_real))+
    ...
    ylab("Realtime Power")+
    xlim(c(0,600))+  # to ignore outliers
    ylim(c(0,20))
